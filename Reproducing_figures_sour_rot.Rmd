...
dfExp1 <- read_excel(path = "Data/TABLE S1_COM1_NEW_prMW.xlsx", skip = 1)
dfExp1 <- dfExp1 |> 
...
dfExp2 <- read_excel(path = "Data/TABLE_S2_COM2_NEW_prMW.xlsx", skip = 1)
...
pl6A <- ggplot(data = dfExp2_Figure6A,  mapping = aes(x = Protection, fill = Order, y = Proportion)) +
  ...
  ggtitle(label = "Wounded berries", subtitle = "Distribution of the order among samples")
...
pl6B <- ggplot(data = dfExp2_Figure6B,  mapping = aes(x = Protection, fill = Genus, y = Proportion)) +
  ...
  ggtitle(label = "Wounded berries", subtitle = "Distribution of the genera among samples")
...
```{r Load data Exp 3}
dfExp3 <- read_excel(path = "Data/TABLE_S3_COM3_NEW_prMW.xlsx", skip = 1)
## removing the raw containing totals
