---
title: "Reproducing the figures of <br>
<i>Revisiting sour rot of grapevine through<br> disease-associated microbiomes:<br> a tripartite co-infection?</i>
 "
...
```{r save and combine Figure 6}
pl6 <- cowplot::plot_grid(pl6A, pl6B, align = "v", nrow = 2, 
                          rel_heights = c(1/2, 1/2), labels = c('', ''))
pl6
ggsave("Figures/Figure6.png", plot = pl6, width = wd, height = 2* hei)
ggsave("Figures/Figure6.eps", plot = pl6, device = cairo_ps, width = wd, height = 2*hei)
...
```{r figure 8 in pdf format}
set.seed(1) # This makes the plot appear the same each time it is run 
heat_tree_sourrot <- heat_tree(obj, 
            node_label = taxon_names,
            node_size = n_obs,
            node_color = obj$data$tax_occ$`sour rot`,
            # title_size = 0.04,
 ...
 set.seed(1) # This makes the plot appear the same each time it is run 
heat_tree_extern <- heat_tree(obj, 
            node_label = taxon_names,
            node_size = n_obs,
            node_color = extern, 
            title_size = 0.04,
...
set.seed(1) # This makes the plot appear the same each time it is run 
heat_tree_drosophilia <- heat_tree(obj, 
            node_label = taxon_names,
            node_size = n_obs,
            node_color = insect, 
            title_size = 0.04,
...
We then combine the plots in a single file and save it in the suitable format.

```{r combine plots figure 8}
bottom_row <- cowplot::plot_grid(heat_tree_extern, heat_tree_drosophilia, labels = c('B', 'C'), label_size = 12)

pl8 <- cowplot::plot_grid(heat_tree_sourrot, bottom_row, labels = c('A', ''), label_size = 12, ncol = 1, rel_heights = c(1/2, 1/2))
ggsave("Figures/Figure8.png", plot = pl8, width = 2*wd, height = 2 * hei)
ggsave("Figures/Figure8.eps", plot = pl8, device = cairo_ps, width = 2*wd, height = 2 * hei)
```
...
Note that the published version of the Figure 9 is similar to `relative_log2_differential_heat_tree.pdf` but with some modifications made with [Inkskape](https://inkscape.org/) to enhance readability and to provide the expected format for publication (`.eps`). But only the sizes of the trees and of the legends have been modified. 
...
We see in particular that the few hits of _Pichia Californica_ found on _Drosophilia_ are only on the endemic ones:

```{r Data Pichia Californica}
dfExp3_Figure10 |> select(Species, Endemic, Suzukii, extern, `sour rot`) |> filter(grepl(pattern = "alifornica",x =  Species))|> knitr::kable()

```

We then proceed with the formatting of data.

```{r finish data formatting Figure 10}

dfExp3_Figure10 <- dfExp3_Figure10 |> 
...
dfExp3_Figure10 <- left_join(dfExp3_Figure10, y = new_cols, by = c("Genus")) |> 
  ...
  mutate(Genus = case_when(type == "Non-Saccharomycotina"  | Genus == "Fungus" ~"Non-Saccharomycotina",
